...
?><div class="cart bundled_item_cart_content" data-title="<?php echo esc_attr( $bundled_item->get_title() ); ?>"
    ...
    data-product_variations="<?php echo htmlspecialchars( json_encode( $bundled_product_variations ) ); ?>"
    data-bundled_item_id="<?php echo $bundled_item->get_id(); ?>"
    data-custom_data="<?php echo esc_attr( json_encode( $custom_product_data ) ); ?>"
    data-product_id="<?php echo $bundled_product_id; ?>" data-bundle_id="<?php echo $bundle_id; ?>"
    <?php echo $bundled_item->is_optional() && ! $bundled_item->is_optional_checked() ? 'style="display:none"' : ''; ?>>
    <table class="variations" cellspacing="0">
        <tbody><?php
			...
			foreach ( $bundled_product_attributes as $attribute_name => $options ) {
                ...
                <td class="label">
                    <label
                        for="<?php echo esc_attr( sanitize_title( $attribute_name ) ) . '_' . $bundled_item->get_id(); ?>"><?php
